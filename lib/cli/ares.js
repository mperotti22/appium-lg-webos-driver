...
const ARES = 'ares';
const ARES_DEVICE_INFO = 'ares-device.js';
const ARES_EXTEND_DEV = 'ares-extend-dev.js';
const ARES_INSTALL = 'ares-install.js';
const ARES_LAUNCH = 'ares-launch.js';
...
export async function getDeviceInfo(deviceName) {
  ...
  const args = ['-i'];
  let {stdout} = await runDeviceCmd(ARES_DEVICE_INFO, deviceName, args);
  stdout = stdout.trim();
